...
class Scenario {
    ...
    Scenario(String name, int totalRequests, boolean applyAlgo1, boolean applyAlgo2, boolean applyAlgo3, boolean applyAlgo4) {
        this.name = name;
        this.totalRequests = totalRequests;
        this.applyAlgo1 = applyAlgo1;
        this.applyAlgo2 = applyAlgo2;
        this.applyAlgo3 = applyAlgo3;
        this.applyAlgo4 = applyAlgo4;
    }
}

public class CloudSimulationLoadBalancerTest {
...
//        runScenario("Scenario 2", true);
    ...
    private static void startSimulation(DatacenterBroker broker, List<Vm> serverList, List<Cloudlet> clientList, ReadOptimizedLoadBalancer loadBalancer) {
        ...
        // Submit cloudlets to the load balancer
        loadBalancer.submitCloudletList(clientList);

        // Start the simulation
        CloudSim.startSimulation();

        // Process delayed write requests at intervals or after certain events
        loadBalancer.processDelayedWriteRequests();

        // Stop the simulation
        CloudSim.stopSimulation();

        // Process any remaining delayed write requests after the simulation
        loadBalancer.processDelayedWriteRequests();
    }
    ...
    private static void measureLatency(DatacenterBroker broker, String scenario) {
        ...
        for (Cloudlet cloudlet : resultList) {
            ...
            if (cloudlet instanceof ReadCloudlet) {
                ...
                System.out.printf("Latency Write: %.2f ms%n", latency);
        ...
        if (readCount > 0) {
            ...
            System.out.printf("Average Read Time - Scenario %s: %.2f ms%n", scenario, averageReadTime);
        }
        if (writeCount > 0) {
            double averageWriteTime = totalWriteTime / writeCount;
            System.out.printf("Average Write Time - Scenario %s: %.2f ms%n", scenario, averageWriteTime);
        }
    }
    ...
    private static void measureThroughput(DatacenterBroker broker, String scenario) {
        ...
        double throughput = totalCompletedCloudlets / simulationTime;  // Throughput calculation cloudlets/unit time. %n
        System.out.printf("Throughput - Scenario %s: %.2f cloudlets/unit time%n", scenario, throughput);
    }
    ...
    private static void measureResourceUtilization(Datacenter datacenter, String scenario) {
        ...
        System.out.printf("Average CPU (MIPS) utilization for %s: %.2f%n", scenario, avgMipsUtilization);
        System.out.printf("Average RAM utilization for %s: %.2f%n", scenario, avgRamUtilization);
        System.out.printf("Average Bandwidth utilization for %s: %.2f%n", scenario, avgBwUtilization);
    }
}

// The rest of the classes and code is as it was provided... and so on
...
class ReadOptimizedLoadBalancer extends LoadBalancer {
    ...
    private int maxWriteRequests = 10; // Threshold for write request throttling
    private Queue<Cloudlet> delayedWriteQueue = new LinkedList<>();
    ...
    public void submitCloudlet(Cloudlet cloudlet) {
        ...
        if (cloudlet instanceof ReadCloudlet) {
            // Adaptive Read Provisioning
            selectedVm = getVmForReadCloudlet(cloudlet);
        } else {
            // Write Request Throttling
            if (canAcceptWriteRequest()) {
                selectedVm = getNextVm();
            } else {
                // Handle write request throttling (e.g., delay or reject)
                handleWriteRequestThrottling(cloudlet);
                return;
            }
        }
    ...
    private Vm getVmForReadCloudlet(Cloudlet cloudlet) {
        // Check if cloudlet is in cache
        if (readCache.containsKey(cloudlet.getCloudletId())) {
            return vmList.get(readCache.get(cloudlet.getCloudletId()));
        }

        Vm selectedVm = null;
        int minWriteCloudlets = Integer.MAX_VALUE;
        for (Vm vm : vmList) {
            int writeCloudletsCount = countWriteCloudlets(vm);
            if (writeCloudletsCount < minWriteCloudlets) {
                minWriteCloudlets = writeCloudletsCount;
                selectedVm = vm;
            }
        }
        ...
        return selectedVm != null ? selectedVm : getNextVm();
    }


    private boolean canAcceptWriteRequest() {
        int totalWriteCloudlets = 0;
        for (Vm vm : vmList) {
            totalWriteCloudlets += countWriteCloudlets(vm);
        }

        return totalWriteCloudlets < maxWriteRequests;
    }


    private void handleWriteRequestThrottling(Cloudlet cloudlet) {
        if (cloudlet instanceof WriteCloudlet) {
            // Queue the write request for later processing
            delayedWriteQueue.add(cloudlet);
        } else {
            // For simplicity, immediately process non-write requests
            Vm vm = getNextVm();
            bindAndSubmit(cloudlet, vm);
        }
    }

    void processDelayedWriteRequests() {
        while (!delayedWriteQueue.isEmpty() && canAcceptWriteRequest()) {
            Cloudlet writeCloudlet = delayedWriteQueue.poll();
            if (writeCloudlet != null) {
                Vm vm = getNextVm();
                bindAndSubmit(writeCloudlet, vm);
            }
        }
    }
